...
func (b *bashInstaller) Install() (ok bool, err error) {
	return executor.NewBashExecutor(b.BashFS.Install()).BindLog(b.File()).Exec().Release()
}

func (b *bashInstaller) Uninstall() (ok bool, err error) {
	return executor.NewBashExecutor(b.BashFS.Uninstall()).BindLog(b.File()).Exec().Release()
}

func (b *bashInstaller) Reinstall() (ok bool, err error) {
	return executor.NewBashExecutor(b.BashFS.Reinstall()).BindLog(b.File()).Exec().Release()
}
