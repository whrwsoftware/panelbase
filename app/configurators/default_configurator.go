...
type defaultConfigurator struct {
	ConfBind  []*appconf.ConfBind
	ConfBindM map[string]*appconf.ConfBind
	Loader    app.ConfigurationLoader
	Cleaner   app.ConfigurationCleaner
}

func DefaultConfigurator(ConfBind []*appconf.ConfBind) *defaultConfigurator {
	var cleanFile []string
	var confBindM = map[string]*appconf.ConfBind{}
	if cbs := ConfBind; cbs != nil {
		for _, cb := range cbs {
			cleanFile = append(cleanFile, cb.Dist)
			confBindM[cb.Name] = cb
		}
	}
	return &defaultConfigurator{ConfBind, confBindM, DefaultConfigurationLoader(), DefaultConfigurationCleaner(cleanFile)}
}

func DefaultConfiguratorParam(confBind []*appconf.ConfBind, confBindM map[string]*appconf.ConfBind, configurationLoader app.ConfigurationLoader, configurationCleaner app.ConfigurationCleaner) app.Configurator {
	return &defaultConfigurator{confBind, confBindM, configurationLoader, configurationCleaner}
}
